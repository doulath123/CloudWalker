...
function UserEdit() {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/posts/"+userid).then((res)=>{
        ...
        userchange(res.user);
        userdescriptionchange(res.userdescription);
        lastacessIPchange(res.lastacessIP);
        fullnamechange(res.fullname);
        mothernamechange(res.mothername);
        productschange(res.products);
        hobbieschange(res.hobbies);
        citychange(res.city);
        statechange(res.state);
        postalcodechange(res.postalcode);
